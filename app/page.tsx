    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-100 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          {session ? (
            <Link
              href="/dashboard"
              className="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition font-medium text-sm"
            >
              Go to Dashboard
            </Link>
          ) : (
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <a
            href="#how-it-works"
            className="border border-gray-200 text-gray-600 px-8 py-4 rounded-xl hover:bg-gray-50 transition font-medium text-lg"
          >
            See How It Works
          </a>
        </div>
          ...
          {[
            {
              ...
              desc: "Drop your PDF resume. We extract and analyze every word.",
            },
            {
              step: "02",
              icon: "🤖",
              title: "AI Roasts It",
              desc: "Claude AI gives you brutally honest, funny, specific feedback in seconds.",
            },
            {
              step: "03",
              icon: "✨",
              title: "Get Better & Apply",
              desc: "See your ATS score, fix suggestions, and download the improved version.",
            },
          ].map((item) => (
            <div key={item.step} className="text-center">
              <div className="text-5xl mb-4">{item.icon}</div>
              <div className="text-xs font-bold text-orange-400 mb-2 tracking-widest">
                STEP {item.step}
              </div>
      ...
      <section className="max-w-2xl mx-auto px-6 py-20 text-center">
        ...
        <p className="text-gray-500 mb-8">
          Free to use. No credit card. Just your resume and a thick skin.
        </p>
        <Link
          href={session ? "/dashboard" : "/auth/signin"}
      ...
      <footer className="border-t border-gray-100 py-8 text-center text-sm text-gray-400">
        <div className="flex items-center justify-center gap-2 mb-2">
          <span>🔥</span>
          <span className="font-semibold text-gray-600">Resume Roaster</span>
        </div>
        <p>Built with Next.js, PostgreSQL & Claude AI</p>
      </footer>
    </div>
  )
}
